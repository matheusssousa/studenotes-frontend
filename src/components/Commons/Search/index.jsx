    ...
    return (
        <div className="search-content">
            <div className="flex w-full md:w-1/2 items-center justify-center gap-1 bg-white rounded-lg">
                ...
                <button type="button" className="btn-search" onClick={() => search.buscar()} title="Pesquisar"><MagnifyingGlass size={17} /></button>
            </div>
            {modalFilters &&
                        ...
                        {search.setSearchStatus &&
                            <span className="input-group-search">
                                ...
                                <select name="status" id="status" className="input-search-date" onChange={(event) => search.setSearchStatus(event.target.value)}>
                                    <option value="" selected={search.status === ""}>Ativos</option>
